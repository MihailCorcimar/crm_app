...
<body>
    ...
    <h1>Encomenda #{{ $order->number }}</h1>
    <div class="meta">
        <div><strong>Data:</strong> {{ optional($order->order_date)->format('Y-m-d') ?? '-' }}</div>
        <div><strong>Validade:</strong> {{ optional($order->valid_until)->format('Y-m-d') ?? '-' }}</div>
        <div><strong>Cliente:</strong> {{ $order->customer?->name ?? '-' }}</div>
        <div><strong>Estado:</strong> {{ $order->status === 'closed' ? 'Fechado' : 'Rascunho' }}</div>
    </div>
        ...
        <tbody>
            @foreach($order->lines as $line)
                <tr>
                    ...
                    <td>{{ number_format((float) $line->sale_price, 2, ',', '.') }} EUR</td>
                    <td>{{ number_format((float) $line->cost_price, 2, ',', '.') }} EUR</td>
                    <td>{{ number_format((float) $line->line_total, 2, ',', '.') }} EUR</td>
                </tr>
            @endforeach
        </tbody>
    </table>

    <div class="total">Total: {{ number_format((float) $order->total, 2, ',', '.') }} EUR</div>
</body>
</html>
